use ethers::{
    middleware::SignerMiddleware, prelude::*, providers::{Http, Provider}, signers::{LocalWallet, Signer as EthersSigner},
};
use eyre::Result;
use std::sync::Arc;
...
async fn main() -> Result<()> {
    ...
    let nonce = forwarder_read.get_nonce(meta_wallet.address()).call().await?;
    ...
    let signature = {
        ...
        let alloy_sig = meta_signer.sign_typed_data(&alloy_struct, &alloy_domain).await?;
    ...
    match tx {
        Err(e) => {
            match e.decode_contract_revert::<abi::forwarder::ForwarderErrors>().expect("Failed to decode contract revert") {
